...
data AnimationEasing
  = Linear
  | Easing AnimationBasicEasing (Maybe AnimationEasingGroup)
  deriving (Generic)
...
instance ToJSON AnimationBasicEasing where
  toJSON Ease      = "ease"
  toJSON EaseIn    = "ease-in"
  toJSON EaseOut   = "ease-out"
  toJSON EaseInOut = "ease-in-out"
...
instance ToJSON AnimationEasing where
   ...
   toJSON (Easing animationEasingBasic animationEasingGroup) =
     case (toJSON <$> animationEasingGroup, toJSON animationEasingBasic) of
       (Just (String g), String b) -> String (b <> "-" <> g)
       (Nothing, b) -> b
       _ -> Null
